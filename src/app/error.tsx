  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-5 py-8">
      <section className="rounded-lg bg-white p-5 shadow-soft">
        ...
        <h1 className="mt-2 text-2xl font-bold text-ink">Something failed while loading</h1>
        <p className="mt-3 text-sm leading-relaxed text-ink/75">
          The app is still running, but this page hit a runtime error. Try again after checking the local server logs.
        </p>
        {showDetails && (
          <pre className="mt-4 max-h-44 overflow-auto rounded-lg bg-clinic p-3 text-xs text-ink">
            {error.message || error.digest || "Unknown runtime error"}
          </pre>
        )}
        <button
          type="button"
